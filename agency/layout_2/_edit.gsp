...
<script>
	...
	$(document).ready(function() {
			...
			<g:if test="${'LICENSE'.equals(params.editType)}">
				...
				$('#agencyIdReadonly, #agencyNameReadonly, #tinReadonly, #agencyLicense\\.0\\.licenseNumber').live('keydown', function(e) {
						if (e.keyCode != 9) {
							e.preventDefault();
						}
				});
				...
				rules : {
					<g:if test="${'ADDRESS'.equals(params.editType)}">
						<g:each in="${agencyInstance?.agentAddresses}" var="agencyAddress" status="i">
								...
								'agencyAddress.${i}.effectiveDate_day' : {
									required : true
								},
								'agencyAddress.${i}.effectiveDate_month' : {
									required : true
								},
								'agencyAddress.${i}.effectiveDate_year' : {
									required : true
								},
								'agencyAddress.${i}.termReason' :  {
									required : function(element){														
										return $.trim($("#agencyAddress\\.${i}\\.termDate_year").val()).length > 0 ||
										$.trim($("#agencyAddress\\.${i}\\.termDate_month").val()).length > 0 ||
										$.trim($("#agencyAddress\\.${i}\\.termDate_day").val()).length > 0;						
									}
								},
								'agencyAddress.${i}.termDate_year' :  {
									required : function(element){														
								...
								'agencyAddress.${i}.termDate_month' :  {
									required : function(element){														
										return $.trim($("#agencyAddress\\.${i}\\.termReason").val()).length > 0 	;						
									}
								},
								'agencyAddress.${i}.termDate_day' :  {
									required : function(element){														
										return $.trim($("#agencyAddress\\.${i}\\.termReason").val()).length > 0 	;						
									}
								},	
...
<style>
	...
	<body>
		<a href="#edit-agencyMaster" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
		...
		<div id="edit-agencyMaster" class="content scaffold-edit" role="main">
			...
			<div class="right-corner" align="center">
				<g:if test="${("MASTER".equals(request.getParameter('editType')))}">AGNCM</g:if>
				<g:if test="${("ADDRESS".equals(request.getParameter('editType')))}">AGNCA</g:if>
				<g:if test="${("CONTACT".equals(request.getParameter('editType')))}">AGNCC</g:if>
