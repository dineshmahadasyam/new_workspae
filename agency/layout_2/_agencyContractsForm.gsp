...
<script type="text/javascript">
	...
	function addContract() {		
		window.location.assign('<g:createLinkTo dir="/agency/addAgencyContract"/>?seqAgencyId=${agencyInstance?.seqAgencyId}');
	}
...
<div id="${agencyInstance?.seqAgencyId}">
	...
	Agency Contract Records for Agency Id (${agencyInstance?.agencyId}) 
	<br/>
	<br/>
	<table border="1" id="report">
		...
		<g:each in="${agencyInstance?.agencyGroupContracts }" var="agencyContract" status="i">
			<input type="hidden" name="seq_contract_id_${i}" value="${agencyContract?.seqAgencyGroupContractId }">
			<input type="hidden" name="agencyContract.${i}.iterationCount" value="${i}">
			<input type="hidden" name="agencyContract.${i}.seqAgencyGroupContractId" value="${agencyContract?.seqAgencyGroupContractId }">
			<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
				<td>
					<input type="radio" name="contractId" class="selectradio" value="${agencyContract?.seqAgencyGroupContractId}">
				</td>
				<td class="clickme">
			...
			<tr class="hideme" id="rowToClone2">
				<td colspan="13">
					<div class="divContent" id="${agencyContract?.seqAgencyGroupContractId}">
						<table class="report1" border="0" style="table-layout: fixed;">
							<tr>
								<td class="tdFormElement" style="white-space: nowrap">
									<div
										...
										<input type="hidden" id="agencyContract.${i}.seqGroupId" name="agencyContract.${i}.seqGroupId" value="${agencyContract.seqGroupId}"/>
																									
										<g:secureTextField name="${i}.groupId" maxlength="30" id = "${i}.groupId" 
											value="${groupMasterMap?.get(agencyContract.seqGroupId).getGroupId()}" tableName="AGENCY_GROUP_CONTRACT"
										...
										<img width="25" height="25" style="float:none;vertical-align:bottom" class="magnifying"
											...
											onclick="lookup('${i}.groupId', 'com.perotsystems.diamond.dao.cdo.GroupMaster','groupId',null, null, 'agencyContract.${i}.seqGroupId', 'seqGroupId' )">
									</div>
								</td>
								<td>
									<div class="fieldcontain">
										...
										<g:textField name="${i}.description" id="${i}.description" value="${groupMasterMap?.get(agencyContract.seqGroupId).getGroupName1()}" readonly="readonly" style="margin-right: 5px;border: none"/>
									</div>
								</td>
							</tr>
							<tr>
								<td class="tdFormElement" style="white-space: nowrap">
									<div class="fieldcontain ${hasErrors(bean: premiumMaster, field: 'tdFormElement', 'error')} ">
										...
										<g:secureTextField name="${i}.planRiderCode" value="${premMasterMap?.get(agencyContract.seqPremId)}" maxlength="30" readonly="readonly"
								...
								<td>
									<div class="fieldcontain ${hasErrors(bean: premiumMaster, field: 'overrideAmount', 'error')} ">
										...
										<g:secureTextField name="agencyContract.${i}.overrideAmount" value="${agencyContract.overrideAmount}" maxlength="30"  onkeypress="return isNumberKey(event)"
							...
							<tr>
								<td colspan="2"><g:actionSubmit class="load" action="deleteContract" value="Delete" onclick="return deleteContract(${agencyContract?.seqAgencyGroupContractId});" />
								</td>
							</tr>
						</table>
