...
<html lang="en">
	<head>
	    ...
	    <script>
			var cdoClassAttributeName2 = "seqPremId"
			var setToIdName = "${setToIdName}"
			var setToIdName1 =  "${setToIdName1}"
			var total = ${cdoCollectionSize};
			var htmlElementName= "${setToIdName}";
			...
			parent.lookupElementtoFocusOnCloseId = "${setToIdName}";
			...
			*/ 
			/*****************************************************/ 
			  ...
			  $(parentBody).click(function(e){
				...
				function populateSeqPremId(seqPremIdValue){
					top.document.getElementById(setToIdName1).value = seqPremIdValue
    		...
    		<div>
				...
				<div class="fms_widget form-inline" style="padding-bottom:0;">
					<form name="searchLookUpForm" onSubmit="fnReloadLookup();return false;">
						...
						<input id="planId" value="${htmlElementValue}" type="text" class="form-control" name="htmlElementValue" readonly="readonly" autofocus="true" aria-labelledby="SearchAgain_label" aria-required="false" autofocus>
       					<button type="button" class="btn btn-primary btn-sm" onclick="fnReloadLookup();document.getElementById('lookupSpinner').style.display='block';"><i class="glyphicon glyphicon-search"></i> Search Again</button>      
      				</form>          
     			</div>
							...
							<div class="fms_table_wrapper0" >
								<table id="BatchIDResults1" class="tablesorter tablesorter-fms tablesorter26ba9e6columnselector" role="grid" aria-describedby="BatchIDResults_pager_info">
									...
									<g:each in="${cdoCollection}" status="j" var="cdoObject">
										...
										<%String searchKeyMethodName2 = "seqPremId";							
											searchKeyMethodName2 = "get"+new String(searchKeyMethodName2.charAt(0)).toUpperCase().concat(searchKeyMethodName2.substring(1, searchKeyMethodName2.length()))	
											java.lang.reflect.Method searchKeyGetMeth2 = cdoObject.getClass().getMethod(searchKeyMethodName2)
										%>	
										...
										<td nowrap="nowrap">
											...
											<button class="btn btn-primary btn-xs SelectBatchID" title="Click to select Value 1." data-dismiss="modal" data-reason="URL 1 - Value 1" onclick="populateSeqPremId(${ searchKeyGetMeth2.invoke(cdoObject) }) ;return fnClose('${searchKeyGetMeth.invoke(cdoObject) }', ${j })">Select</button>&nbsp;${searchKeyGetMeth.invoke(cdoObject) }
											<div style="display:none">							
											<input type="radio" name="myLookUpValue" value="${searchKeyGetMeth.invoke(cdoObject) }" onclick=""/>&nbsp;&nbsp;${ searchKeyGetMeth.invoke(cdoObject) }
											</div>
											<input type="hidden" id="${ searchKeyGetMeth.invoke(cdoObject) }" value="${ searchKeyGetMeth.invoke(cdoObject) }"/>
