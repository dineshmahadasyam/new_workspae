		...
		function fnClose() {
			...
			if (updateFieldsCdoName) {
				if (updateFieldsCdoName.indexOf("|") != -1) {
					...
					var fieldToUpdate = document.getElementsByName(updateFieldsCdoName)
					//alert (updateFields && updateFields.indexOf("|") != -1)
					if (updateFields && updateFields.indexOf("|") == -1) {
						top.document.getElementById(updateFields).value = fieldToUpdate[selectedIndex].value
					}
...
<body>
	...
	<form name="searchLookUpForm" onSubmit="fnReloadLookup();return false;">
		&nbsp;&nbsp;Find: &nbsp;&nbsp;<input type="text" id="idSrchBatchId"
			name="srchBatchId" value="${htmlElementValue}"> &nbsp;&nbsp;<input
			class="search" type="button" name="Search" value="Search"
			onclick="fnReloadLookup()">
	</form>
	<br>
	<form name="lookUpMainForm">
		<div style="width: 100%; height: 125px; overflow-y: scroll;">
			<table border="1">
				<thead>
					<tr>
						<th>Select</th>
						...
						<%String searchKeyMethodName = cdoClassAttributeName							
							...
							<%  if (updateFieldsCdoName) {
								if (updateFieldsCdoName.indexOf("|") != -1) {
									...
									%>
									<input type="hidden" name="${ cdoAttributeName }" id="${ cdoAttributeName }" value="${ searchKeyGetMeth1.invoke(cdoObject) }"/>
								<%
									}
								} else {
								...
								java.lang.reflect.Method searchKeyGetMeth1 = cdoObject.getClass().getMethod(searchKeyMethodName1)
							%>
							<input type="hidden" name="${ updateFieldsCdoName }"    id="${ updateFieldsCdoName }" value="${ searchKeyGetMeth1.invoke(cdoObject) }"/>
						<%
								}
							}
								 %>														
