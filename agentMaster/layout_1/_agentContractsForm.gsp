...
function addContract() {
	...
	window.location.assign("/"+ appName+"/agentMaster/addAgentContract?seqAgentId=" + seqAgentId);
}
				...
				$('.groupIdAlertInfo').click(function(e){ 
			 	...
			 	$('#SearchAlertModalInfo').modal({
			      ...
			      $("#BtnDeleteRowYes").click(function(e){	  		         
			               ...
			               myFm.action = "/"+appName+"/agentMaster/deleteContract";     	
			               myFm.submit();
...
<div id="fms_content_body">
	...
	<g:if test="${agentMasterInstance?.seqAgentId}">
				...
				<table id="DataTable"
					...
					<thead>
						<tr role="row" class="tablesorter-headerRow">
							...
							<th class="{sorter: false} tablesorter-header sorter-false"	data-column="4" data-columnselector="disable" aria-disabled="true">Plan/Rider Code</th>

							<th data-columnselector="disable" class="{sorter: false} tablesorter-header sorter-false tablesorter-headerUnSorted" data-column="6" scope="col" role="columnheader"
								aria-disabled="true" unselectable="on" aria-sort="none"	style="-webkit-user-select: none;">
								<div class="tablesorter-header-inner">Actions</th>
						</tr>
					</thead>
					<tfoot></tfoot>

					<tbody aria-live="polite" aria-relevant="all">
						<g:each in="${agentMasterInstance?.agentGroupContracts }"
							var="agentContract" status="i">
							<input type="hidden" name="seq_contract_id_${i}"
								value="${agentContract?.seqAgencyGroupContractId }">
							<input type="hidden" name="agentContract.${i}.iterationCount"
								value="${i}">
							<input type="hidden"
								name="agentContract.${i}.seqAgencyGroupContractId"
								value="${agentContract?.seqAgencyGroupContractId }">
							...
							<tr id="Row2Child" class="tablesorter-childRow" role="row">
								<td colspan="3" id="${agentContract?.seqAgencyGroupContractId}"	class="" style="display: none;">
											...
											<div class="fms_form_layout_2column">
												...
												<div class="fms_form_column fms_long_labels">
													...
													<div class="fms_form_input">
														<input type="hidden" id="agentContract.${i}.seqGroupId"
																name="agentContract.${i}.seqGroupId"
																value="${agentContract.seqGroupId}"
																onchange="resetPlanCode(${i})"
																onblur="resetPlanCode(${i})" />
														<g:secureTextField name="${i}.groupId" maxlength="30"
																	id="${i}.groupId" class="form-control"
																	value="${groupMasterMap?.get(agentContract.seqGroupId).getGroupId()}"
														...
														<fmsui:cdoLookup lookupElementId="${i}.groupId"
																	...
																	lookupElementValue="${groupMasterMap?.get(agentContract.seqGroupId).getGroupId()}"
													...
													<div class="fms_form_input fms_has_feedback">
														<input type="hidden" id="agentContract.${i}.seqPremId"
																name="agentContract.${i}.seqPremId"	value="${agentContract.seqPremId}" />
														<g:secureTextField name="${i}.planRiderCode"
																		value="${premMasterMap?.get(agentContract.seqPremId)}"
																		maxlength="30" readonly="readonly" tableName="AGENCY_GROUP_CONTRACT"
																		attributeName="seqPremId" class="form-control"
																		aria-labelledby="planRiderCode_r2_label"  
																		aria-describedby="planRiderCode_r2_error" aria-required="false">
														</g:secureTextField>
														<button type="button" id="${i}.planRiderCode"
												...
												<div class="fms_form_column fms_long_labels">
													...
													<div class="fms_form_input">
														<g:textField name="${i}.description" id="${i}.description"
															value="${groupMasterMap?.get(agentContract.seqGroupId).getGroupName1()}"
															readonly="readonly" class="form-control" aria-labelledby="description_r2_label"  
													...
													<div class="fms_form_input">
														<g:secureTextField
															name="agentContract.${i}.overrideAmount"
															value="${agentContract.overrideAmount}" maxlength="30"
												...
												<div class="fms_form_button">
														<button id="BtnAddressDelete2" type="button"
															...
															onclick="return deleteContract(${agentContract?.seqAgencyGroupContractId});">
